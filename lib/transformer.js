...
class Transformer {
    ...
    getImportZodObjectProperties() {
        let zodObjectPropertiesImportStatement = `import type { ZodObjectProperties } from '${process.env.NODE_ENV === 'development'
            ? '../../../..'
            : 'prisma-zod-generator'}/lib/types';`;
    ...
    getImportsForObjectSchemas(withZodObjectProperties) {
    ...
    addExportObjectSchema(schema) {
        ...
        return `const Schema = ${schema};\n\n ${end}`.replace('const Schema = z.object', `const Schema = z.object<ZodObjectProperties<Prisma.${name}>>`);
    }
    ...
    getFinalForm(zodStringFields) {
        ...
        return `${this.getImportsForObjectSchemas(objectSchema.indexOf('ZodObjectProperties') !== -1)}${prismaImport}${json}${objectSchema}`;
    }
